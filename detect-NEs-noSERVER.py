...
def are_match(stanford_NEs, gazetteer_NEs):
	if len(stanford_NEs)!=len(gazetteer_NEs):
		return False
	for item_ind in range(len(stanford_NEs)):
...
en_file_name = '../English Text/pc_en.txt'
...
for ind in xrange(1000000,len(valid_lines)):
	...
	if len(valid_lines[ind])>=1:# and valid_lines[ind][0][0]!='Taliban':
		...
		if are_match(tagged_words_stanford,valid_lines[ind]):
